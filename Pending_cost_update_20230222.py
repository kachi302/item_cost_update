...
try:
    ...
    # 전체 작업 시간
    start1 = time.time()
    job = pyautogui.confirm('Pending Cost Update 작업을 진행 하시겠습니끼?', buttons= ['OK','Cancel']) # type: ignore    
    ...
    if job == 'OK':
        ...
        for row in ws.iter_rows():
            ...
            start = time.time()
            item = row[0].value
            cost_type = row[1].value
            cost = row[2].value
            overhead = row[3].value
            ...
            if item_mp is not None:
                ...
                time.sleep(0.7)
                # cost type 입력
                pyautogui.write(str(cost_type), interval=0.1)  
                time.sleep(0.7)
                pyautogui.hotkey('tab')  
                # Find Button Click
                time.sleep(0.7)
        ...
        if std_menu is not None:
            ...
            for row_f in ws.iter_rows():
                ...
                # cost_type = row_f[1].value
                cost_type = 'Pending'
                if cost_type not in ['Pending', 'Annual'] : break
                
                x = dt.datetime.now()                
                # print(x.strftime('%Y%m%d%H%M'))
                cost_remark = 'KRK_'+ str(item_code)+'_'+x.strftime('%Y%m%d%H%M')            
                time.sleep(1)    
                ...
                if update_menu is not None:
                    ...
                    pyautogui.press('tab', presses=2, interval=0.3)
                    time.sleep(0.3)
                    pyautogui.write(str(item_code), interval=0.1)
                    time.sleep(0.3)
                    pyautogui.hotkey('tab')
                    time.sleep(0.3)
                    pyautogui.hotkey('enter')
